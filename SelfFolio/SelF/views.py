...
def grade(request):
    if request.method == 'GET':
        ...
        spi_sem1 = 0
        if total11 > 0:
            spi_sem1 = total1/total11
        total_credits_earned = total1
        total_credits = total11
        cpi_sem1 = 0
        if total_credits > 0:
            cpi_sem1 = total_credits_earned/total_credits
        ...
        spi_sem2 = 0
        if total22 > 0:
            spi_sem2 = total2 / total22
        total_credits_earned += total2
        total_credits += total22
        cpi_sem2 = 0
        if total_credits > 0:
            cpi_sem2 = total_credits_earned / total_credits
        ...
        spi_sem3 = 0
        if total33 > 0:
            spi_sem3 = total3 / total33
        total_credits_earned += total3
        total_credits += total33
        cpi_sem3 = 0
        if total_credits > 0:
            cpi_sem3 = total_credits_earned / total_credits
        ...
        spi_sem4 = 0
        if total44 > 0:
            spi_sem4 = total4 / total44
        total_credits_earned += total4
        total_credits += total44
        cpi_sem4 = 0
        if total_credits > 0:
            cpi_sem4 = total_credits_earned / total_credits
        ...
        spi_sem5 = 0
        if total55 > 0:
            spi_sem5 = total5 / total55
        total_credits_earned += total5
        total_credits += total55
        cpi_sem5 = 0
        if total_credits > 0:
            cpi_sem5 = total_credits_earned / total_credits
        ...
        spi_sem6 = 0
        if total66 > 0:
            spi_sem6 = total6 / total66
        total_credits_earned += total6
        total_credits += total66
        cpi_sem6 = 0
        if total_credits > 0:
            cpi_sem6 = total_credits_earned / total_credits
        ...
        spi_sem7 = 0
        if total77 > 0:
            spi_sem7 = total7 / total77
        total_credits_earned += total7
        total_credits += total77
        cpi_sem7 = 0
        if total_credits > 0:
            cpi_sem7 = total_credits_earned / total_credits
        ...
        spi_sem8 = 0
        if total88 > 0:
            spi_sem8 = total8 / total88
        total_credits_earned += total8
        total_credits += total88
        cpi_sem8 = 0
        if total_credits > 0:
            cpi_sem8 = total_credits_earned / total_credits
...
def reminders(request):
    if request.method == 'GET':
        user_id = request.user.id
        reminders = Reminders.objects.filter(userProfile=user_id)
        currReminders = reminders.filter(dateNtime__gte=datetime.datetime.now())
        oldReminders = reminders.filter(dateNtime__lt=datetime.datetime.now())
        return render(request, 'reminders.html', {
            'reminders': reminders,
            'currReminders': currReminders,
            'oldReminders': oldReminders,
        })
...
def addReminder(request):
    ...
    if request.method == 'GET':
        ...
        #dateNtime = request.POST['dateNtime']
        form = addReminders(request.POST)
        if form.is_valid():
            user_id = request.user.id
            user = User.objects.filter(id=user_id)
            code = form.cleaned_data.get('code')
            ...
            dateNtime = datetime.datetime.combine(date, time)
            ...
            print(form.errors)
            messages.error(request, "Invalid form entries.")
            return redirect('addReminder')


@login_required(login_url='login_page')
def editReminder(request):
    reminder_id = request.GET.get('id')
    reminder = Reminders.objects.filter(id=reminder_id)
    form = addReminders(initial={
        'code': reminder[0].code,
        'subject': reminder[0].subject,
        'task': reminder[0].task,
        'date': reminder[0].dateNtime.date(),
        'time': reminder[0].dateNtime.time(),
    })
    if request.method == 'GET':
        return render(request, 'addReminder.html', {
            'form': form
...
def deleteReminder(request):
    ...
    reminder = Reminders.objects.filter(id=reminder_id)
    if request.method == 'GET':
        return render(request, 'deleteReminder.html', {
            'reminder': reminder[0],
        })
    else:
        reminder.delete()
